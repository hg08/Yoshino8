...
tw_list = [0, 16, 32, 64, 128, 256, 512, 1024]
tw_list_M20 = [0, 4096, 8192, 65536, 262144, 524288, 1048576, 2097152]
tw_list_test = [0,1,2,4,8,16,32,64]
...
class Network:
    def __init__(self,init,tw,L,M,N,N_in,N_out,tot_steps,beta,timestamp):
        ...
        self.S = np.load('../data/{:s}/seed_S_{:s}_init{:d}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}.npy'.format(str_timestamp,str_timestamp,init,tw,L,M,N,beta))
        self.J_hidden = np.load('../data/{:s}/seed_J_hidden_{:s}_init{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}.npy'.format(str_timestamp,str_timestamp,init,tw,L,N,beta))
        self.J_in = np.load('{}/{:s}/seed_J_in_{:s}_init{:d}_tw{:d}_N{:d}_N_in{:d}_beta{:4.2f}.npy'.format(data_dir, str_timestamp, str_timestamp, init, tw, N, N_in, beta))
        self.J_out = np.load('{}/{:s}/seed_J_out_{:s}_init{:d}_tw{:d}_N_out{:d}_N{:d}_beta{:4.2f}.npy'.format(data_dir,str_timestamp,str_timestamp,init,tw,N_out,N,beta))
        ...
        #=============================
        # For testing
        self.sq_energy_diff_from_node = 0
        self.sq_energy_diff_from_bond = 0
        #=============================
        self.node_active = 0
        self.bond_active = 0
    def ave_energy_diff_induced_by_flip(self):
        """
        This function give the average energy difference induced by flipping a node.
        """
        res = np.sqrt(self.sq_energy_diff_from_node) /(self.node_active)
        print("Average ener diff induced from flip = {}".format(res))
        return res
    def ave_energy_diff_induced_by_shift(self):
        """
        This function give the average energy difference induced by shifting a hidden bond.
        """
        #res = np.sqrt(self.sq_energy_diff_from_bond) /(self.num_bonds * self.tot_steps)
        res = np.sqrt(self.sq_energy_diff_from_bond) /(self.bond_active)
        print("Average ener diff induced from shift = {}".format(res))
        return res
    ...
    def check_and_save_seeds(self,str_timestamp):
        ...
        if ind==self.list_k[0]:
            self.list_k.pop(0) 
            np.save('../data/{:s}/seed_S_{:s}_init{:d}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}.npy'.format(str_timestamp,str_timestamp,init,ind,self.L,self.M,self.N,self.beta),self.S)   
            np.save('../data/{:s}/seed_J_hidden_{:s}_init{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}.npy'.format(str_timestamp,str_timestamp,init,ind,self.L,self.N,self.beta),self.J_hidden)
            np.save('../data/{:s}/seed_J_in_{:s}_init{:d}_tw{:d}_N{:d}_N_in{:d}_beta{:4.2f}.npy'.format(str_timestamp,str_timestamp,init,ind,self.N,self.N_in,self.beta),self.J_in)
            np.save('../data/{:s}/seed_J_out_{:s}_init{:d}_tw{:d}_N_out{:d}_N{:d}_beta{:4.2f}.npy'.format(str_timestamp,str_timestamp,init,ind,self.N_out,self.N,self.beta),self.J_out)
            np.save('../data/{:s}/seed_ener_new_{:s}_init{:d}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}.npy'.format(str_timestamp,str_timestamp,init,ind,self.L,self.M,self.N,self.beta),self.H_hidden)
        else:
            pass
    def check_and_save(self):
        ...
        if 2**self.ind_save == ind and self.ind_save < self.S_traj.shape[0]:
            ...
            pass
    def check_and_save_seeds(self,update_mc_index,str_timestamp):
        ind = update_mc_index
        init = self.init
    ...
    def decision_by_mu_l_n_SIMPLE(self,mu,l,n):
        ...
        #print("ener(a2) for part_gap_hidden_after_flip:")
        #print(calc_ener(a2))
        #print("ener(a1) for part_gap_hidden_before_flip:")
        #print(calc_ener(a1))
    ...
    def decision_by_mu_l_n(self,mu,l,n,EPS,rand):
        ...
        if delta_e < EPS:
            ...
            #TEST (2 line)
            self.sq_energy_diff_from_node = self.sq_energy_diff_from_node + delta_e**2
            self.node_active = self.node_active + 1
            prob_ref = np.exp(-delta_e * self.beta)
            if rand < prob_ref:
                self.accept_by_mu_l_n(mu,l,n)
                #print("[S] Delta E:{:12.10f}".format(delta_e))
                print("Delta_E (node hidden): {}".format(delta_e))
                print("The prob. from rand number (node hidden): {}".format(prob_ref))
            else:
                pass
    def decision_node_in_by_mu_n(self,mu,n,EPS,rand):
        ...
        if delta_e < EPS:
            ...
            self.node_active = self.node_active + 1
            prob_ref = np.exp(-delta_e * self.beta)
            print("Delta_E (node in): {}".format(delta_e))
            print("The prob. from rand number (node in): {}".format(prob_ref))
            if rand < prob_ref:
		#==========================================================================================================================================================
                #I print the prob for TESTING. From the rand number to check if it is too small. If it too small, the MC process will be extremly slow, this is not I want.
		#==========================================================================================================================================================
                self.accept_by_mu_l_n(mu,temp_l,n)
            else:
                pass
    def decision_node_out_by_mu_n(self,mu,n,EPS,rand):
        ...
        if delta_e < EPS:
            ...
            self.node_active = self.node_active + 1
            prob_ref = np.exp(-delta_e * self.beta)
            print("Delta_E (node out): {}".format(delta_e))
            print("The prob. from rand number (node out): {}".format(prob_ref))
            if rand < prob_ref:
                self.accept_by_mu_l_n(mu,l,n)
            else:
                pass
    ...
    def decision_bond_hidden_by_l_n2_n1(self,l,n2,n1,EPS,rand):
        self.delta_H = calc_ener(self.part_gap_hidden_after_shift_2(l)) - calc_ener(self.part_gap_hidden_before_shift_2(l))
        delta_e = self.delta_H
        #==============================================================================
        self.sq_energy_diff_from_bond = self.sq_energy_diff_from_bond + delta_e**2
        #==============================================================================
        if delta_e < EPS:
            ...
            #TEST (2 line)
            self.sq_energy_diff_from_bond = self.sq_energy_diff_from_bond + delta_e**2
            self.bond_active = self.bond_active + 1
            prob_ref = np.exp(-delta_e * self.beta)
            print("The prob. from rand number (bond hidden): {}".format(prob_ref))
            print("[J] Delta E:{:12.10f}".format(delta_e))
            if rand < prob_ref:
                self.accept_bond_hidden_by_l_n2_n1(l,n2,n1)
                #print("[J] Delta E:{:12.10f}".format(delta_e))
    ...
    def decision_bond_in_by_n2_n1(self,n2,n1,EPS,rand):
        ...
        #print("a2")
        #print(a2)
        #print("ener(a2) for part_gap_in_after_shift:")
        #print(calc_ener(a2))
        #print("ener(a1) for part_gap_in_before_shift:")
        #print(calc_ener(a1))
        self.delta_H = calc_ener(a2) - calc_ener(a1)
        print("delta_e:")
        print(self.delta_H)
        ...
        if delta_e < EPS:
            ...
            self.bond_active = self.bond_active + 1
            prob_ref = np.exp(-delta_e * self.beta)
            print("The prob. from rand number (bond in): {}".format(prob_ref))
            print("[J] Delta E:{:12.10f}".format(delta_e))
            if rand < prob_ref:
                self.accept_bond_in_by_n2_n1(n2,n1)
            else:
    ...
    def decision_bond_out_by_n2_n1(self,n2,n1,EPS,rand):
        ...
        #print("a2")
        #print(a2)
        #print("ener(a2) for part_gap_out_after_shift:")
        #print(calc_ener(a2))
        #print("ener(a1) for part_gap_out_before_shift:")
        #print(calc_ener(a1))
        self.delta_H = calc_ener(a2) - calc_ener(a1)
        print("delta_e:")
        print(self.delta_H)
        ...
        if delta_e < EPS:
            ...
            self.bond_active = self.bond_active + 1
            prob_ref = np.exp(-delta_e * self.beta)
            print("The prob. from rand number (bond out): {}".format(prob_ref))
            print("[J] Delta E:{:12.10f}".format(delta_e))
            if rand < prob_ref:
                self.accept_bond_out_by_n2_n1(n2,n1)
            else:
    ...
    def mc_main(self,str_timestamp,replica_index):
        ...
        rel_path='J_hidden_{:s}_init{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}_host.dat'.format(str_replica_index,self.init,self.tw,self.L,self.N,self.beta,self.tot_steps)
    ...
    def mc_odd(self,replica_index):
        ...
        str_replica_index = str(replica_index)
        rel_path='J_hidden_{:s}_init{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}_odd.dat'.format(str_replica_index,self.init,self.tw,self.L,self.N,self.beta,self.tot_steps)
        src_dir = os.path.dirname(__file__) 
        abs_file_path=os.path.join(src_dir, rel_path)
        #file1 = open(abs_file_path,'w')
    ...
    def mc_even(self,replica_index):
        """MC for guest machines, i.e., it will not save seeds for different waiting time."""
        str_replica_index = str(replica_index)
        rel_path='J_hidden_{:s}_init{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}_even.dat'.format(str_replica_index,self.init,self.tw,self.L,self.N,self.beta,self.tot_steps)
        src_dir = os.path.dirname(__file__) 
        abs_file_path=os.path.join(src_dir, rel_path)
        #file1 = open(abs_file_path,'w')
        ...
        for MC_index in range(1,self.tot_steps):
            ...
            for update_index in range(0, self.num_variables):
                ...
                if l == 0:
                    self.decision_node_in_by_mu_n(mu,n,EPS,p1)
                elif l == self.num_hidden_node_layers - 1:

                    self.decision_node_out_by_mu_n(mu,n,EPS,p1)
                else:
                    self.decision_by_mu_l_n(mu,l,n,EPS,p1)
            self.count_MC_step += 1
            #Check and save the configureation of bonds 
            self.check_and_save() # save configuration
    ...
    def mc_main_hyperfine(self,str_timestamp,replica_index):
        ...
        for MC_index in range(1,self.tot_steps):
            ...
            for update_index in range(start_index_variables, end_index_variables):
                ...
                #Check and save the seeds 
                self.check_and_save_seeds(update_index,str_timestamp)
    ...
    @timethis
    def mc_main_random_update_hyperfine_2(self,str_timestamp):
        """MC for the guest machine, i.e., it will save seeds for different waiting time.
        ...
        for mc_index in range(MC_STEPS):
            ...
            for inner_mc_index in range(self.num):
                self.mc_update_J_or_S()
                #Check and save the configureation of variables
                self.update_index = self.update_index + 1 
                self.check_and_save_hyperfine(self.update_index) # save configuration

            #Check and save the seeds (ONLY IN mc_main) 
            self.check_and_save_seeds(mc_index,str_timestamp)
    ...
    def mc_main_random_update_hyperfine(self,str_timestamp):
        """1. MC for the main machine, i.e., it will save seeds for different waiting time.
