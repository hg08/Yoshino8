...
n_replica = 8
#tw_list = [0, 8192, 8192, 65536, 262144, 524288, 1048576, 2097152]
...
if __name__ == '__main__':
    ...
    para_list = [[ L, M, N, N_in, N_out, tot_steps_list[0], tw_list[0], init],
        ...
        [L,       M,        N,      N_in,       N_out,    tot_steps_list[7], tw_list[7], init]]
    ...
    for index_tw, tw in enumerate(tw_list):
        grand_J[index_tw] = np.load('{}/{}/overlap_J_ave_over_init_{:s}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,timestamp_list[i],timestamp_list[i],tw,L,N,beta,tot_steps_list[index_tw]))
        grand_S[index_tw] = np.load('{}/{}/overlap_S_ave_over_init_{:s}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,timestamp_list[i],timestamp_list[i],tw,L,M,N,beta,tot_steps_list[index_tw]))
    ...
    np.save('{}/{:s}/grand_J_{:s}_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,timestamp_list[i],timestamp_list[i],L,M,N,beta,tot_steps),grand_J)
    np.save('{}/{:s}/grand_S_{:s}_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,timestamp_list[i],timestamp_list[i],L,M,N,beta,tot_steps),grand_S)
