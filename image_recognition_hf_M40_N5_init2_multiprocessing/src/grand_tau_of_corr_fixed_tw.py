...
M_list = [20, 30, 40, 50, 60, 70, 80, 90]
...
if __name__ == '__main__':
    ...
    tot_steps_list = [2048,2048,2048,4096,4096,4096,8192,8192]
    SQ_N = N ** 2
    ...
    for index_M,term in enumerate(M_list):
        #ggrand_tau_J[index_M] = np.load('/public1/home/sc91981/image_recognition_hf_M{:d}_N{:d}_init{:d}_multiprocessing/data1/grand_tau_J_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(term,N,init,L,term,N,beta,tot_steps_list[index_M]))
        ggrand_tau_S[index_M] = np.load('/public1/home/sc91981/image_recognition_hf_M{:d}_N{:d}_init{:d}_multiprocessing/data1/grand_tau_S_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(term,N,init,L,term,N,beta,tot_steps_list[index_M]))
    ...
    #======================================
    #Define which layers we want to look at
    selected_l_list = [1,4,7] 
    #======================================
    for l_index,term in enumerate(tw_list): 
        print("shape of ggrand_tau_J:")
        print(ggrand_tau_J.shape)
        print("shape of ggrand_tau_S:")
        print(ggrand_tau_S.shape)
        #plot_ggrand_tau_J_tw_ave_over_init_fixed_tw(ggrand_tau_J,selected_l_list,L,N,beta,alpha_list,tw_list)
        plot_ggrand_tau_S_tw_ave_over_init_fixed_tw(ggrand_tau_S,selected_l_list,L,N,beta,alpha_list,tw_list,term)
